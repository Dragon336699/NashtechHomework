...
namespace BankSimulationMVC.Services
{
    public class AccountService : IAccountService
    {
        ...
        {
            Account? account = await _unitOfWork.Accounts.FirstOrDefaultAsync(acc => acc.AccountNumber == accountNumber);
        ...
        {
            var existAccount = await _unitOfWork.Accounts.FirstOrDefaultAsync(a => a.AccountNumber == account.AccountNumber);
        ...
        {
            Account? account = await _unitOfWork.Accounts.FirstOrDefaultAsync(a => a.AccountNumber == depositViewModel.AccountNumber);
            if (account == null)
            {
                return new ServiceResult
                {
        ...
        {
            Account? account = await _unitOfWork.Accounts.FirstOrDefaultAsync(a => a.AccountNumber == withdrawViewModel.AccountNumber);
            if (account == null)
            {
                return new ServiceResult
                {
        ...
        public async Task<ServiceResult> Transfer(TransferVM transferViewModel)
        {
            try
            {
                ...
                _unitOfWork.Transactions.Add(logTransactionSource);
                _unitOfWork.Transactions.Add(logTransactionDes);
        ...
        {
            Account? account = await _unitOfWork.Accounts.FirstOrDefaultAsync(a => a.AccountNumber == accountNumber);
